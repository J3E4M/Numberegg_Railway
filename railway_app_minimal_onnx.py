# Railway Minimal ONNX Egg Detection API
# Downloads ONNX model at runtime - smallest possible image
...
app = FastAPI(title="NumberEgg Minimal ONNX API", version="1.0.0", lifespan=lifespan)
...
async def init_onnx():
    """Initialize ONNX model - download if not present"""
...
async def root():
    """Root endpoint"""
    return {"message": "NumberEgg Minimal ONNX API is running", "version": "1.0.0", "model": "YOLOv8n ONNX"}
...
def postprocess_outputs(outputs, original_shape, input_shape=(640, 640)):
    ...
    for i in range(len(boxes)):
        if final_scores[i] > 0.25:  # Lower confidence threshold for better detection
            x1, y1, x2, y2 = boxes[i]
